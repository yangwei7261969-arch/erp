...
export default function BianfeiListPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('确定要删除这条编菲记录吗？')) return;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold">编菲管理</h1>
            <p className="text-muted-foreground">管理和查询编菲记录</p>
          </div>
        </div>
              ...
              {filteredRecords.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-12 text-muted-foreground">
                    {records.length === 0 ? '暂无编菲记录，点击"新增编菲"创建第一条记录' : '没有匹配的记录'}
                  </TableCell>
                </TableRow>
              ) : (
